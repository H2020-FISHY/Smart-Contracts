...
class Command(BaseCommand):
    help = 'Retry to validate events of the FISHY components'
    ...
    def write_to_rabbitmq(self, event_id, type, ipfs_link, tx_hash, error_message):
        ...
        channel.queue_bind(exchange=settings.SMART_CONTRACTS_RABBITMQ_EXCHANGE, queue=connection.queue_name, routing_key=settings.SMART_CONTRACTS_RABBITMQ_ROUTING_KEY_VALIDATE)
    ...
    def parse(self):
        ...
        result = channel.queue_declare(queue='', exclusive=True, durable=True)
        connection.queue_name = result.method.queue
        channel.exchange_declare(exchange=settings.SMART_CONTRACTS_RABBITMQ_EXCHANGE, exchange_type='direct')
        channel.queue_bind(exchange=settings.SMART_CONTRACTS_RABBITMQ_EXCHANGE, queue=connection.queue_name, routing_key=settings.SMART_CONTRACTS_RABBITMQ_ROUTING_KEY_RETRY)
